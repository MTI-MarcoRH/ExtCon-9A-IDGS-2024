-- Scrip de la Creacion de Triggers (BEFORE_INSERT, BEFORE_UPDATE, AFTER_UPDATE, AFTER_DELETE )
...
-- Triggers para la tabla Aprobaciones
-- 1) BEFORE_INSERT
------------------------------------------------------------------------------------------------------
DELIMITER &&
...
-- 2) BEFORE UPDATE
-- tbb_aprobaciones_BEFORE_UPDATE
------------------------------------------------------------------------------------------------------
DELIMITER &&
...
-- 3) AFTER UPDATE
-- tbb_aprobaciones_AFTER_UPDATE
------------------------------------------------------------------------------------------------------

DELIMITER &&

CREATE DEFINER=`carlos.crespo`@`%` TRIGGER `tbb_aprobaciones_AFTER_UPDATE` AFTER UPDATE ON `tbb_aprobaciones` FOR EACH ROW BEGIN
...
-- 4) AFTER DELETE
-- tbb_aprobaciones_AFTER_DELETE
------------------------------------------------------------------------------------------------------
DELIMITER &&

CREATE DEFINER=`carlos.crespo`@`%` TRIGGER `tbb_aprobaciones_AFTER_DELETE` AFTER DELETE ON `tbb_aprobaciones` FOR EACH ROW BEGIN
    -- Declaración de variables
    DECLARE v_estatus_descripcion VARCHAR(20) DEFAULT 'En Proceso';
    DECLARE v_tipo_solicitud VARCHAR(20) DEFAULT 'Servicio Interno';
    DECLARE personal_medico VARCHAR(200) DEFAULT 'No Aplica';
